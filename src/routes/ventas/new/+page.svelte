			...
			switch (result.type) {
				case 'success':
					if (result.data.error) {
						...
						toast.error("Error al crear el metodo de pago", { duration: 7000});
						break;
					}
					await invalidateAll();
...
<div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
		...
		<form action="?/create" method="POST" enctype="multipart/form-data" use:enhance={enhanceNew} >
			<div class="grid gap-4 mb-4 sm:grid-cols-2">
				<div>
					<Input id="nombre" label="Nombre" value={form?.data?.nombre} errors={formerrors?.nombre} placeholder="Nombre del metodo de pago" />
				</div>
				<div>
					<Input id="descuento" label="Descuento" value={form?.data?.descuento} errors={formerrors?.descuento} placeholder="0" type="number" />
				</div>
				<div class="mt-1">
				...
				<div class="sm:col-span-2">
					<TextArea rows="4" id="observaciones" label="Descripción" placeholder="Ingrese una breve descripción del metodopago" value={form?.data?.observaciones} errors={form?.errors?.observaciones}/>
				</div>
			</div>
