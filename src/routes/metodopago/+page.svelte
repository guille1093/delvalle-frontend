<script>
	...
	const tableheaditems = [
		{id: 1, titulo: "Nombre", icon: "bx-card" },
		{id: 2, titulo: "Descuento", icon: "bx-calendar" },
		...
		return async ({ result, update }) => {
			switch (result.type) {
				case 'success':
					...
					break;
				case 'error':
					toast.error('No se pudo eliminar el metodo de pago. Intente nuevamente');
					break;
				default:
					await update();
			}
	...
	$: filteredItems = data.metodopagos.filter((metodopago) => {
		const searchTerms = searchTerm.toLowerCase().split(' ');
		for (let i = 0; i < searchTerms.length; i++) {
			const term = searchTerms[i];
			if (
					//Todos los campos que se pueden filtrar
				metodopago.nombre.toLowerCase().indexOf(term) !== -1 ||
				metodopago.descuento.toString().toLowerCase().indexOf(term) !== -1 ||
				metodopago.observaciones.toLowerCase().indexOf(term) !== -1 ||
				metodopago.updated.toLowerCase().indexOf(term) !== -1 ||
				metodopago.created.toLowerCase().indexOf(term) !== -1 ||
				metodopago.estado.toLowerCase().indexOf(term) !== -1
			) {
				// si se encuentra un resultado, se establece el ID del metodopago
				// y se devuelve verdadero para incluir el resultado en la lista de resultados
				if (!filteredItems || filteredItems.length === 0) {
					filteredItems = [metodopago];
				} else if (filteredItems[0].id === metodopago.id) {
					filteredItems.push(metodopago);
				}
				return true;
			}
		}
			...
			<div class="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
				<div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
					...
					<div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
						<Button href="/metodopago/new" class="ml-4 ">
						...
						<Tooltip placement="top">
							Registre un nuevo metodo de pago para asociar a una venta
						</Tooltip>
					</div>
				</div>
			</div>
		...
		<TableBody class="divide-y">
			{#each filteredItems as item}
				<TableBodyRow>
					<TableBodyCell>
						<div class="h-2.5">
							<a href="/metodopago/{item.id}">{item.nombre} </a>
						</div>
					</TableBodyCell>
					<TableBodyCell>
						<div class="text-lg font-thin items-end w-max  max-h-2.5 ">
							<i>% {pesoARLocale.format(item.descuento)}</i>
						</div>
					</TableBodyCell>
					<TableBodyCell>
					...
					<TableBodyCell>
						...
						<Modal label={item.id} checked={modalOpen}>
							...
							<div slot="actions" class="flex w-full items-center justify-center space-x-2">
								<label
									...
									>Cancelar</label
								>
								<form action="?/deletemetodopago" method="POST" use:enhance={submitDeletemetodopago}>
						...
						<a class="bx bx-edit text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 rounded text-sm px-5 py-2.5 align-middle mr-1 mb-1 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
								href="/metodopago/{item.id}/edit"> </a>
						<Tooltip placement="top">
							Editar {item.nombre}
						</Tooltip>
					</TableBodyCell>
